 ...
 * Markdown 变换的端到端测试。
 *
 * 用真实的 Transformer 组装（createMarkdownTransformer），而不是分别测每个函数——
 * 管道顺序本身是设计的一部分（见 transforms.ts 顶部注释），必须一起锁住。
 * 两个口味开关这里全开，以覆盖最长的执行路径；开关行为在 config.test.ts 里测。
 */

const transform = createMarkdownTransformer({ common: true, deCircled: true });
const run = (
  md: string,
...
test("围栏标志行识别", () => {
  assert.ok(isCodeFenceChromeLine("```text"));
  assert.ok(isCodeFenceChromeLine("```"));
  assert.ok(isCodeFenceChromeLine("```ts"));
  assert.ok(isCodeFenceChromeLine("\x1b[36m```text\x1b[0m"));
  assert.ok(!isCodeFenceChromeLine("  ```text"));
  assert.ok(!isCodeFenceChromeLine("foo ```text"));
});

test("藏掉代码块围栏，保留内容与块后空行", () => {
  const out = hideCodeFenceChrome(["```text", "  foo", "```", ""]);
