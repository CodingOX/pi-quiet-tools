...
test("session replacement disposes the runtime-scoped global display API", async () => {
  const displayApiKey = Symbol.for("pi-tool-display-intent.api.v1");
  const globalState = globalThis as Record<symbol, unknown>;
  const previousApi = globalState[displayApiKey];
  const { api, capturedHandlers } = createApiStub();
  toolDisplayExtension(api);
  assert.ok((globalThis as Record<symbol, unknown>)[displayApiKey]);

  for (const { handler } of capturedHandlers.filter(({ event }) => event === "session_shutdown")) {
    await handler({ reason: "new" });
  }

  assert.equal(globalState[displayApiKey], previousApi);
});
...
test("runtime ownership survives host and child shutdown in either order", async () => {
  const runtimeOwnersKey = Symbol.for("pi-tool-display-intent.runtime-owners.v1");
  const host = createApiStub();
  const child = createApiStub();
  const replacement = createApiStub();
  const shutdown = async (stub: ReturnType<typeof createApiStub>, reason: string) => {
    for (const captured of stub.capturedHandlers.filter((handler) => handler.event === "session_shutdown")) {
      await captured.handler({ reason });
    }
  };

  toolDisplayExtension(host.api);
  toolDisplayExtension(child.api);
  const owners = (globalThis as Record<symbol, unknown>)[runtimeOwnersKey] as WeakSet<object>;
  assert.ok(owners.has(host.api));
  assert.ok(owners.has(child.api));

  await shutdown(host, "new");
  assert.equal(owners.has(host.api), false);
  assert.equal(owners.has(child.api), true);

  toolDisplayExtension(replacement.api);
  assert.equal(owners.has(replacement.api), true);
  await shutdown(child, "complete");
  assert.equal(owners.has(child.api), false);
  assert.equal(owners.has(replacement.api), true);

  await shutdown(replacement, "reload");
  assert.equal(owners.has(replacement.api), false);
});
