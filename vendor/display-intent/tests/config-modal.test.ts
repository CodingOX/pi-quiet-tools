...
test("layout command validates values, saves, and prompts for reload", async () => {
	...
	await getHandler()!("layout per-turn", ctx);
	assert.equal(getLastSet().config?.toolCallLayout, "per-turn");
	...
	assert.match(invalidContext.notifications[0]?.message ?? "", /individual\|aggregate\|per-turn/);
});
...
test("per-turn modal hides individual-only settings and describes tool phases", () => {
	const retained = {
		...DEFAULT_TOOL_DISPLAY_CONFIG,
		toolCallLayout: "per-turn" as const,
		resultMode: "preview" as const,
		previewRows: 40,
	};
	const settings = buildInspectorSettings(retained, {
		hasMcpTooling: false,
		hasRtkOptimizer: false,
	});
	assert.deepEqual(settings.map((setting) => setting.id), ["toolCallLayout"]);
	const layoutSummary = settings[0]?.inspectorSummary.join(" ") ?? "";
	assert.match(layoutSummary, /one tool-phase ledger/);
	assert.match(layoutSummary, /retained but inactive/);
	assert.ok(settings[0]?.inspectorOptions?.some((option) => option.startsWith("per-turn")));
});
