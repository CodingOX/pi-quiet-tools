...
test("5: child shutdown preserves the active host user-message patch", async () => {
  const host = createApiStub();
  const child = createApiStub();
  const config = { ...DEFAULT_TOOL_DISPLAY_CONFIG, toolCallLayout: "aggregate" } as ToolDisplayConfig;
  const prototype = UserMessageComponent.prototype as PatchableUserMessagePrototype;
  const emit = async (stub: ReturnType<typeof createApiStub>, event: string, ctx?: unknown) => {
    for (const captured of stub.capturedHandlers.filter((entry) => entry.event === event)) {
      await captured.handler({}, ctx);
    }
  };

  registerNativeUserMessageBox(host.api, () => config);
  registerNativeUserMessageBox(child.api, () => config);
  try {
    await emit(host, "session_start", { hasUI: true, ui: { theme: stubTheme } });
    const hostRender = prototype.render;

    await emit(child, "session_start", { hasUI: false });
    await emit(child, "session_shutdown");

    assert.equal(prototype.render, hostRender, "child cleanup must not restore the host-owned prototype");
    assert.ok(prototype.__piUserMessageNativePatched);
  } finally {
