...
function createProjection(grouping: "user" | "tool-phase" = "user"): AggregateProjection {
	return new AggregateProjection((toolName) =>
		(DEFAULT_AGGREGATE_RENDER_PASSTHROUGH as readonly string[]).includes(toolName), grouping);
}
...
test("normalizeAssistantNarration keeps markdown structure and drops invisible reasoning", () => {
	assert.equal(
		normalizeAssistantNarration("先对照 **两边入口**\u0007\n\n`src/index.ts`"),
		"先对照 **两边入口**\n\n`src/index.ts`",
	);
	assert.equal(normalizeAssistantNarration("<thinking>hidden reasoning</thinking>"), undefined);
	assert.equal(normalizeAssistantNarration("<thinking>streaming reasoning"), undefined);
	assert.equal(
		normalizeAssistantNarration("<thinking-note>visible narration</thinking-note>"),
		"<thinking-note>visible narration</thinking-note>",
	);
	assert.equal(
		normalizeAssistantNarration("<thinking>hidden reasoning</thinking>\n继续核对入口"),
		"继续核对入口",
	);
	assert.equal(normalizeAssistantNarration("   \n\n  "), undefined);
});

test("per-turn grouping ignores serialized hidden GPT thinking", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-hidden-thinking");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-phase-one",
		stopReason: "toolUse",
		content: [{ type: "toolCall", ...call("read-before-thinking", "read", { path: "a.ts" }) }],
	});
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-hidden-thinking",
		stopReason: "toolUse",
		content: [{ type: "text", text: "<thinking>reviewing tool output" }],
	});
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-phase-two",
		stopReason: "toolUse",
		content: [{ type: "toolCall", ...call("read-after-thinking", "read", { path: "b.ts" }) }],
	});
	projection.markStarted("read-before-thinking", "read", { path: "a.ts" });
	projection.markStarted("read-after-thinking", "read", { path: "b.ts" });

	const first = projection.getMember("read-before-thinking");
	const second = projection.getView("read-after-thinking");
	assert.equal(first?.groupId, second?.groupId);
	assert.equal(second?.callCount, 2);
	assert.equal(second?.agentTurnCount, 2);
	assert.equal(projection.getGroups().filter((group) => group.leaderToolCallId).length, 1);
});
...
test("settled Tools ledger shows duration, tokens, cache, and completion time under the header", () => {
	...
	assert.doesNotMatch(rendered[0] ?? "", /took /);
	assert.equal(rendered[1], `  took 2m14s · tok ↑62k ↓8.4k R120k W4.1k · at ${formatAggregateClock(endedAt)}`);
	assert.doesNotMatch(rendered.join("\n"), /›/);
});

test("open Tools ledger shows ticking open elapsed on the collapsed header", () => {
	const view: Parameters<typeof renderAggregateActivity>[0] = {
		groupId: "open-elapsed",
		leaderToolCallId: "tool-open",
		hasRunning: true,
		callCount: 1,
		agentTurnCount: 1,
		settled: false,
		...
		steerCount: 0,
		pinnedSteers: [],
		toolSummaries: [{ toolName: "read", count: 1, lastTarget: "Read(a.ts)" }],
	};
	const rendered = renderAggregateActivity(view, 120, plainTheme());
	assert.match(rendered[0] ?? "", /Tools \(1 call · 1 turn\) · 12s/);
	assert.doesNotMatch(rendered.join("\n"), /took /);

	const settled: Parameters<typeof renderAggregateActivity>[0] = {
		...view,
		hasRunning: false,
		settled: true,
		durationMs: 12_000,
	...
	const settledRendered = renderAggregateActivity(settled, 120, plainTheme());
	assert.match(settledRendered[0] ?? "", /Tools \(1 call · 1 turn\)/);
	assert.doesNotMatch(settledRendered[0] ?? "", /12s|took /);
	assert.match(settledRendered[1] ?? "", /took 12s/);
});

test("open elapsed uses accent so it stands apart from muted header stats", () => {
	const view: Parameters<typeof renderAggregateActivity>[0] = {
		groupId: "open-elapsed-color",
		leaderToolCallId: "tool-open-color",
		hasRunning: true,
		callCount: 1,
		agentTurnCount: 1,
		settled: false,
	...
	const rendered = renderAggregateActivity(view, 120, {
		fg: (color, text) => `[${color}]${text}`,
	});
	assert.match(rendered[0] ?? "", /\[muted\] \(1 call · 1 turn\)/);
	assert.match(rendered[0] ?? "", /\[muted\] · \[accent\]12s/);
	assert.doesNotMatch(rendered[0] ?? "", /\[muted\]12s/);
	assert.match(rendered[0] ?? "", /\[warning\]read ×1/);
});

test("steered open Tools ledger keeps open elapsed off the receipt line", () => {
	const startedAt = Date.now() - 5_000;
	const projection = createProjection();
	projection.startUserGroup("user-open-steer", startedAt);
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-open-steer",
		stopReason: "toolUse",
		timestamp: startedAt + 1_000,
		content: [{ type: "toolCall", ...call("read-open-steer", "read", { path: "a.ts" }) }],
	});
	projection.markStarted("read-open-steer", "read", { path: "a.ts" });
	projection.ingestUserMessage({
		role: "user",
		content: "先确定方案",
		timestamp: startedAt + 2_000,
	}, { streamingBehavior: "steer" });
	const view = projection.getView("read-open-steer");
	assert.equal(view?.settled, false);
	assert.equal(view?.startedAtMs, startedAt);
	const rendered = renderAggregateActivity(view!, 160, plainTheme());
	assert.match(rendered[0] ?? "", /Tools \(1 call · 1 turn\) · 5s/);
	assert.doesNotMatch(rendered.join("\n"), /took /);
});

test("open elapsed timer starts while a group is open and stops on settle or shutdown", async () => {
	const projection = createProjection();
	...
	(globalThis as typeof globalThis).setInterval = ((fn: () => void, ms?: number) => {
		intervalCallback = fn;
		intervalMs = ms;
		intervalCount += 1;
		return fakeHandle;
	}) as typeof setInterval;
	(globalThis as typeof globalThis).clearInterval = ((handle: ReturnType<typeof setInterval>) => {
		if (handle === fakeHandle) clearedCount += 1;
	...
	try {
		...
		await handlers.get("message_start")?.[0]?.({
			message: { role: "user", content: "no tools yet" },
		});
		assert.equal(intervalCount, 0);
		await handlers.get("message_update")?.[0]?.({
			message: {
				...
				id: "timer-assistant",
				stopReason: "toolUse",
				content: [{ type: "toolCall", ...call("timer-read", "read", { path: "a.ts" }) }],
			},
		});
		assert.equal(intervalCount, 1);
		await handlers.get("tool_execution_start")?.[0]?.({
			toolCallId: "timer-read",
			toolName: "read",
			args: { path: "a.ts" },
		});
		assert.equal(intervalCount, 1);
		assert.equal(intervalMs, 1_000);
		assert.ok(projection.getGroups().some((group) => !group.settled));
		let invalidationCount = 0;
		const invalidateOpenLedgers = projection.invalidateOpenLedgers.bind(projection);
		projection.invalidateOpenLedgers = () => {
			invalidationCount += 1;
			invalidateOpenLedgers();
		};
		intervalCallback?.();
		...
		const clearedAfterSettle = clearedCount;

		await handlers.get("tool_execution_start")?.[0]?.({
			toolCallId: "timer-read-2",
			toolName: "read",
			args: { path: "b.ts" },
		});
		assert.ok(intervalCount >= 2);
		await handlers.get("session_shutdown")?.[0]?.({ reason: "reload" });
		assert.ok(clearedCount > clearedAfterSettle);
	} finally {
		(globalThis as typeof globalThis).setInterval = originalSetInterval;
		(globalThis as typeof globalThis).clearInterval = originalClearInterval;
		await handlers.get("session_shutdown")?.[0]?.({ reason: "reload" });
		restoreAggregateToolExecutions();
	}
});

test("settled successful ledger uses muted visual chrome and no trailing gap", () => {
	const colors: string[] = [];
	let boldCalls = 0;
	const theme = {
	...
	const view: Parameters<typeof renderAggregateActivity>[0] = {
		groupId: "quiet-ledger",
		leaderToolCallId: "tool-quiet",
		hasRunning: false,
		callCount: 1,
		agentTurnCount: 1,
		settled: true,
	...
	renderAggregateActivity(view, 120, theme);
	assert.ok(colors.length > 0);
	assert.ok(colors.every((color) => color === "muted"));
	assert.equal(boldCalls, 0);
	assert.deepEqual(padAggregateBlock(["quiet ledger"]), ["", "quiet ledger", ""]);
	assert.deepEqual(padAggregateBlock([]), []);
});
...
test("per-turn grouping keeps streaming calls in a narrated next batch together", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-per-turn");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-read",
		stopReason: "toolUse",
	...
	projection.markStarted("read-1", "read", { path: "a.ts" });
	projection.markComplete("read-1", { content: [{ type: "text", text: "ok" }] }, false);
	projection.markStarted("read-2", "read", { path: "b.ts" });
	...
	const narratedBatch = {
		...
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "接下来改这里" },
			{ type: "toolCall", ...call("edit-1", "edit", { path: "a.ts" }) },
		],
	};
	projection.ingestAssistantMessage(narratedBatch);
	projection.ingestAssistantMessage({
		...narratedBatch,
		content: [
			...narratedBatch.content,
			{ type: "toolCall", ...call("edit-2", "edit", { path: "b.ts" }) },
		],
	});
	const first = projection.getView("read-2");
	const second = projection.getView("edit-2");
	assert.equal(projection.getView("read-1"), undefined);
	assert.notEqual(first?.groupId, second?.groupId);
	assert.equal(first?.callCount, 2);
	assert.equal(second?.callCount, 2);
	assert.equal(first?.agentTurnCount, 1);
	assert.equal(second?.agentTurnCount, 1);
	assert.equal(projection.getGroups().filter((group) => group.leaderToolCallId).length, 2);
});

test("per-turn grouping keeps a continuous tool phase together across assistant messages", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-tool-phases");

	for (const [index, name] of ["read", "bash", "ls"].entries()) {
		const id = `phase-one-${index + 1}`;
		projection.ingestAssistantMessage({
			role: "assistant",
			id: `assistant-phase-one-${index + 1}`,
			stopReason: "toolUse",
			content: [{ type: "toolCall", ...call(id, name) }],
		});
		projection.markStarted(id, name, {});
		projection.markComplete(id, { content: [{ type: "text", text: "ok" }] }, false);
	}

	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-phase-one-final",
		stopReason: "stop",
		content: [{ type: "text", text: "第一阶段完成" }],
	});

	for (const [index, name] of ["bash", "read", "find"].entries()) {
		const id = `phase-two-${index + 1}`;
		projection.ingestAssistantMessage({
			role: "assistant",
			id: `assistant-phase-two-${index + 1}`,
			stopReason: "toolUse",
			content: [{ type: "toolCall", ...call(id, name) }],
		});
		projection.markStarted(id, name, {});
	...
	const first = projection.getView("phase-one-3");
	const second = projection.getView("phase-two-3");
	assert.equal(first?.callCount, 3);
	assert.equal(second?.callCount, 3);
	assert.equal(first?.agentTurnCount, 3);
	assert.equal(second?.agentTurnCount, 3);
	assert.notEqual(first?.groupId, second?.groupId);
	assert.equal(projection.getGroups().filter((group) => group.leaderToolCallId).length, 2);
});

test("per-turn grouping starts a new phase when a boundary message repeats an old call before a new one", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-repeated-call-boundary");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-phase-one",
		stopReason: "toolUse",
		content: [{ type: "toolCall", ...call("phase-one-call", "read", { path: "a.ts" }) }],
	});
	projection.markStarted("phase-one-call", "read", { path: "a.ts" });
	projection.markComplete("phase-one-call", { content: [{ type: "text", text: "ok" }] }, false);

	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-boundary",
		stopReason: "stop",
		content: [{ type: "text", text: "先看完这一阶段" }],
	});
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-phase-two",
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "接下来继续处理" },
			{ type: "toolCall", ...call("phase-one-call", "read", { path: "a.ts" }) },
			{ type: "toolCall", ...call("phase-two-call", "edit", { path: "a.ts" }) },
		],
	});

	const first = projection.getView("phase-one-call");
	const second = projection.getView("phase-two-call");
...
test("per-turn grouping does not count a later text-only assistant as a second turn", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-turn-count");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-read",
		stopReason: "toolUse",
	...
	projection.ingestAssistantMessage({
		...
		id: "assistant-final",
		stopReason: "stop",
		content: [{ type: "text", text: "介绍写完了" }],
	});
	assert.equal(projection.getView("read-1")?.agentTurnCount, 1);
});

test("per-turn grouping does not count the next batch's text-only stream on the previous ledger", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-next-stream");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-read",
		stopReason: "toolUse",
	...
	projection.ingestAssistantMessage({
		...
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "目前可以确认" },
			{ type: "toolCall", ...call("read-2", "read", { path: "b.ts" }) },
		],
	});
	assert.equal(projection.getView("read-1")?.agentTurnCount, 1);
	assert.equal(projection.getView("read-2")?.agentTurnCount, 1);
});

test("per-turn grouping keeps streaming updates of the same assistant message on one ledger", () => {
	const projection = createProjection("tool-phase");
	projection.startUserGroup("user-stream");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-stream",
		stopReason: "pending",
	...
	projection.ingestAssistantMessage({
		...
		content: [
			{ type: "text", text: "先定位两边的设计与实现入口" },
			{ type: "toolCall", ...call("read-stream", "read", { path: "a.ts" }) },
			{ type: "toolCall", ...call("read-stream-2", "read", { path: "b.ts" }) },
		],
	});
	const view = projection.getView("read-stream-2");
	assert.equal(view?.callCount, 2);
	assert.equal(view?.agentTurnCount, 1);
	assert.equal(projection.getGroups().filter((group) => group.leaderToolCallId).length, 1);
});

test("per-turn rebuild keeps a steer on the previous batch and opens a new ledger for the next tools", () => {
	const projection = createProjection("tool-phase");
	const branch = [
	...
	projection.rebuild(branch, messages(branch));
	const first = projection.getView("read-1");
	const second = projection.getView("edit-1");
	assert.notEqual(first?.groupId, second?.groupId);
	assert.equal(first?.callCount, 1);
	assert.equal(first?.steerCount, 1);
	assert.equal(second?.callCount, 1);
	assert.equal(second?.steerCount, 0);
	assert.equal(projection.getGroups().filter((group) => group.leaderToolCallId).length, 2);
});
...
test("session replacement releases the old host ledger for the rebound runtime", async () => {
	initTheme("dark", false);
	const oldHost = createProjection();
	const child = createProjection();
	const replacement = createProjection();
	const oldHandlers = new Map<string, Array<(event: any, ctx?: any) => unknown>>();
	const childHandlers = new Map<string, Array<(event: any, ctx?: any) => unknown>>();
	const replacementHandlers = new Map<string, Array<(event: any, ctx?: any) => unknown>>();
	const createApi = (handlers: Map<string, Array<(event: any, ctx?: any) => unknown>>) =>
		({
			on(event: string, handler: (event: any, ctx?: any) => unknown) {
				handlers.set(event, [...(handlers.get(event) ?? []), handler]);
			},
		}) as unknown as ExtensionAPI;
	const oldApi = createApi(oldHandlers);
	const childApi = createApi(childHandlers);
	const replacementApi = createApi(replacementHandlers);
	const emptyUiContext = {
		hasUI: true,
		sessionManager: {
			getBranch: () => [],
			buildSessionContext: () => ({ messages: [] }),
	...
	registerAggregateProjectionEvents(oldApi, oldHost);
	try {
		await oldHandlers.get("session_start")?.[0]?.({ reason: "startup" }, emptyUiContext);
		oldHost.startUserGroup("old-user");
		oldHost.markStarted("old-read", "read", { path: "src/old.ts" });
		assert.equal(getActiveAggregateProjection(), oldHost);

		registerAggregateProjectionEvents(childApi, child);
		await childHandlers.get("session_start")?.[0]?.({ reason: "startup" }, childContext);
		child.startUserGroup("child-user");
		child.markStarted("child-read", "read", { path: "src/child.ts" });

		await oldHandlers.get("session_shutdown")?.[0]?.({ reason: "new" });

		const individualRow = createComponent("read", "individual-read", { path: "src/individual.ts" });
		assert.match(
			individualRow.render(120).join("\n"),
			/ORIGINAL read/,
			"unmatched calls must not inherit the destroyed aggregate host fallback",
		);
		const childRow = createComponent("read", "child-read", { path: "src/child.ts" });
		assert.match(childRow.render(120).join("\n"), /Tools.*read ×1/);
		registerAggregateProjectionEvents(replacementApi, replacement);
		await replacementHandlers.get("session_start")?.[0]?.({ reason: "new" }, emptyUiContext);
		replacement.startUserGroup("replacement-user");
		replacement.markStarted("replacement-read", "read", { path: "src/new.ts" });

		assert.equal(getActiveAggregateProjection(), replacement);
		const row = createComponent("read", "replacement-read", { path: "src/new.ts" });
		assert.match(row.render(120).join("\n"), /Tools.*read ×1/);

		await childHandlers.get("session_shutdown")?.[0]?.({ reason: "complete" });
		assert.equal(getActiveAggregateProjection(), replacement);
		assert.match(row.render(120).join("\n"), /Tools.*read ×1/);
	} finally {
		await replacementHandlers.get("session_shutdown")?.[0]?.({ reason: "reload" });
		await oldHandlers.get("session_shutdown")?.[0]?.({ reason: "reload" });
		restoreAggregateToolExecutions();
	}
});

test("per-turn narration detects only a preceding Tools ledger", () => {
	const projection = createProjection("tool-phase");
	...
	projection.ingestAssistantMessage({
		...
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "先定位入口" },
			{ type: "toolCall", ...call("read-opening", "read", { path: "a.ts" }) },
		],
	});
	assert.equal(projection.hasPrecedingToolsLedger("assistant-before:read-opening"), false);
	...
	projection.ingestAssistantMessage({
		...
		content: [
			...
			{ type: "toolCall", ...call("read-after-tools", "read", { path: "b.ts" }) },
		],
	});
	assert.equal(projection.hasPrecedingToolsLedger("assistant-before:read-after-tools"), true);

	projection.startUserGroup("user-next");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-next-user",
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "新问题的开场说明" },
			{ type: "toolCall", ...call("read-next-user", "read", { path: "c.ts" }) },
		],
	});
	assert.equal(projection.hasPrecedingToolsLedger("assistant-before:read-next-user"), false);
});

test("user grouping keeps narration before its latest Tools ledger", () => {
	const projection = createProjection("user");
	projection.startUserGroup("user-grouping");
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-first-tool",
		stopReason: "toolUse",
		content: [{ type: "toolCall", ...call("read-first", "read", { path: "a.ts" }) }],
	});
	projection.ingestAssistantMessage({
		role: "assistant",
		id: "assistant-before-latest-tool",
		stopReason: "toolUse",
		content: [
			{ type: "text", text: "此旁白位于最新工具之前" },
			{ type: "toolCall", ...call("read-latest", "read", { path: "b.ts" }) },
		],
	});

	assert.equal(projection.hasPrecedingToolsLedger("assistant-before:read-latest"), false);
});
