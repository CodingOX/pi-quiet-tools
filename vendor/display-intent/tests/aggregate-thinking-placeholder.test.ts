...
test("thinking placeholder predicate follows the active UI runtime owner", async () => {
	initTheme("dark", false);
	const createRuntime = () => {
		const handlers = new Map<string, Array<(event: unknown, ctx?: { hasUI?: boolean }) => unknown>>();
		const api = {
			on(event: string, handler: (event: unknown, ctx?: { hasUI?: boolean }) => unknown) {
		...
		return {
			api,
			async emit(event: string, ctx?: { hasUI?: boolean }) {
				for (const handler of handlers.get(event) ?? []) await handler({}, ctx);
			},
		};
	};
	const message = assistant([
		{ type: "thinking", thinking: "runtime-specific reasoning" },
		{ type: "toolCall", id: "read-owner", name: "read", arguments: { path: "a.ts" } },
	]);
	const assertHidden = () => assert.deepEqual(render(message, true), []);
	const assertVisible = () => assert.match(render(message, true).join("\n"), /Thinking\.\.\./);
	const host = createRuntime();
	const child = createRuntime();
	const peerHost = createRuntime();

	restoreAggregateThinkingPlaceholders();
	try {
		registerAggregateThinkingPlaceholderSuppression(host.api, () => true);
		await host.emit("session_start", { hasUI: true });
		assertHidden();

		registerAggregateThinkingPlaceholderSuppression(child.api, () => false);
		await child.emit("session_start", { hasUI: false });
		assertHidden();
		await child.emit("session_shutdown", { hasUI: false });
		assertHidden();

		registerAggregateThinkingPlaceholderSuppression(peerHost.api, () => false);
		await peerHost.emit("session_start", { hasUI: true });
		assertVisible();
		await peerHost.emit("session_shutdown", { hasUI: true });
		assertHidden();
	} finally {
		await child.emit("session_shutdown", { hasUI: false });
		await peerHost.emit("session_shutdown", { hasUI: true });
		await host.emit("session_shutdown", { hasUI: true });
		restoreAggregateThinkingPlaceholders();
	}
});
