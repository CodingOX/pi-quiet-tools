...
import { usesAggregateLedger, type ToolDisplayConfig } from "./types.js";
import { onSessionShutdown } from "./extension-lifecycle.js";

interface NativeUserMessageOwner {
...
const nativeUserMessageOwnersByApi = new WeakMap<ExtensionAPI, NativeUserMessageOwner>();
const nativeUserMessageOwners = new Set<NativeUserMessageOwner>();
let activeNativeUserMessageOwner: NativeUserMessageOwner | undefined;
...
function activateNativeUserMessageOwner(owner: NativeUserMessageOwner): void {
  if (activeNativeUserMessageOwner !== owner) {
    if (activeNativeUserMessageOwner) restoreUserMessageRender();
    activeNativeUserMessageOwner = owner;
  }
  patchUserMessageRender(owner.getTheme, owner.isEnabled, owner.isCompact);
}

function relinquishNativeUserMessageOwner(owner: NativeUserMessageOwner): void {
  if (activeNativeUserMessageOwner !== owner) return;
  restoreUserMessageRender();
  activeNativeUserMessageOwner = undefined;
  const fallback = [...nativeUserMessageOwners].reverse().find((candidate) => candidate.hasUi === true);
  if (fallback) activateNativeUserMessageOwner(fallback);
}
...
export default function registerNativeUserMessageBox(
  ...
  if (nativeUserMessageOwnersByApi.has(pi)) return;

  let activeTheme: UserMessageTheme | undefined;
  const owner: NativeUserMessageOwner = {
    getTheme: () => activeTheme,
    isEnabled: () => usesAggregateLedger(getConfig().toolCallLayout) || getConfig().enableNativeUserMessageBox,
    isCompact: () => usesAggregateLedger(getConfig().toolCallLayout),
  };
  nativeUserMessageOwnersByApi.set(pi, owner);
  nativeUserMessageOwners.add(owner);
  if (!activeNativeUserMessageOwner) activateNativeUserMessageOwner(owner);

  onSessionShutdown(pi, () => {
    nativeUserMessageOwnersByApi.delete(pi);
    nativeUserMessageOwners.delete(owner);
    activeTheme = undefined;
    relinquishNativeUserMessageOwner(owner);
  });

  pi.on("before_agent_start", async (_event, ctx) => {
    if (ctx?.hasUI !== undefined) owner.hasUi = ctx.hasUI !== false;
    if (ctx?.hasUI !== false) activateNativeUserMessageOwner(owner);
    else relinquishNativeUserMessageOwner(owner);
  });

  pi.on("session_start", async (_event, ctx) => {
    activeTheme = ctx?.ui?.theme as unknown as UserMessageTheme;
    owner.hasUi = ctx?.hasUI !== false;
    if (owner.hasUi) activateNativeUserMessageOwner(owner);
    else relinquishNativeUserMessageOwner(owner);
  });
}
