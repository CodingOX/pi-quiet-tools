...
import type { DisposableScope } from "./disposable.js";
import { onSessionShutdown } from "./extension-lifecycle.js";
...
export interface AggregateGroup {
	groupId: string;
	/** 同一用户请求内的连续工具阶段共享该链标识。 */
	toolPhaseChainId: string;
...
const GPT_THINKING_BLOCK_PATTERN = /<thinking(?=[\s>])[^>]*>[\s\S]*?<\/thinking\s*>/gi;
const GPT_UNCLOSED_THINKING_PATTERN = /<thinking(?=[\s>])[^>]*>[\s\S]*$/i;
...
export function normalizeAssistantNarration(value: unknown): string | undefined {
	if (typeof value !== "string") return undefined;
	// GPT-compatible gateways may serialize hidden reasoning as ordinary text.
	// The aggregate boundary must use the same visible-text semantics as the UI;
	// otherwise invisible reasoning can split one continuous tool phase in two.
...
function forgetProjection(owner: object | undefined, projection: AggregateProjection): void {
	...
	const patchState = getToolExecutionPrototype()[AGGREGATE_TOOL_EXECUTION_PATCH_KEY];
	if (patchState?.projection === projection) patchState.projection = undefined;
}
...
/**
 * 供外层渲染补丁判断当前中途旁白是否紧随已输出的 Tools ledger。
 */
export function hasPrecedingAggregateToolsLedger(message: unknown): boolean {
	const frameId = aggregateAssistantFrameId(message);
	return Boolean(frameId && hostAggregateProjection?.hasPrecedingToolsLedger(frameId));
}
...
export class AggregateProjection {
	...
	private toolPhaseBoundaryPending = false;
	private lastAssistantToolMessageId: string | undefined;
	private initialized = false;
	private renderTheme: AggregateRenderTheme | undefined;

	constructor(
		private readonly isPassthroughTool: (toolName: string) => boolean = () => false,
		private readonly grouping: "user" | "tool-phase" = "user",
	) {}
	...
	/**
	 * 判断一个中途旁白帧之前是否已有可见 Tools ledger。
	 * 同组帧按 transcript 顺序排列；新的 tool phase 则继承前一组 ledger。
	 */
	hasPrecedingToolsLedger(frameId: string): boolean {
		const groupId = this.framedGroupById.get(frameId);
		if (!groupId) return false;
		const groupIndex = this.groups.findIndex((group) => group.groupId === groupId);
		const group = this.groupsById.get(groupId);
		if (!group || groupIndex < 0) return false;

		const frameIndex = group.framedItemIds.indexOf(frameId);
		const leaderIndex = group.leaderToolCallId
			? group.framedItemIds.indexOf(group.leaderToolCallId)
			: -1;
		if (leaderIndex >= 0 && leaderIndex < frameIndex) {
			return true;
		}

		return this.groups.slice(0, groupIndex).some((previous) => {
			return (
				previous.toolPhaseChainId === group.toolPhaseChainId &&
				Boolean(previous.leaderToolCallId)
			);
		});
	}
	...
	rememberAgentTurn(message: unknown): void {
		...
		const hasTools = toolCallsFromMessage(message).length > 0;
		if (this.grouping !== "tool-phase" || hasTools) {
	...
	startUserGroup(
		...
		options: {
			...
			interruptUnsettled?: boolean;
			continueToolPhase?: boolean;
		} = {},
	): string {
		const previousGroupId = this.activeGroupId;
		const previousGroup = previousGroupId ? this.groupsById.get(previousGroupId) : undefined;
		if (previousGroupId && previousGroupId !== groupId) {
			if (options.interruptUnsettled !== false) this.markUnsettledInterrupted();
		...
		if (options.continueToolPhase && previousGroup) {
			group.toolPhaseChainId = previousGroup.toolPhaseChainId;
		}
		this.activeGroupId = resolvedId;
		this.initialized = true;
		this.toolPhaseBoundaryPending = false;
		this.lastAssistantToolMessageId = undefined;
	...
	ingestUserMessage(
		...
		if (this.shouldTreatAsSteer(options.streamingBehavior)) {
			...
			if (this.grouping === "tool-phase") this.toolPhaseBoundaryPending = true;
			return "steer";
		}
	...
	ingestAssistantMessage(message: unknown): void {
		...
		this.maybeStartToolPhaseGroup(message);
	...
	invalidateOpenLedgers(): void {
		for (const group of this.groups) {
			if (group.settled || !group.leaderToolCallId || !this.getView(group.leaderToolCallId)) continue;
			this.invalidateIds(group.leaderToolCallId);
		}
	}

	hasOpenLedger(): boolean {
		return this.groups.some(
			(group) => !group.settled
				&& group.leaderToolCallId !== undefined
				&& this.getView(group.leaderToolCallId) !== undefined,
		);
	}

	private maybeStartToolPhaseGroup(message: unknown): void {
		if (this.grouping !== "tool-phase") return;
		const calls = toolCallsFromMessage(message);
		const hasNarration = messageHasVisibleText(message);
		const activeGroup = this.activeGroupId ? this.groupsById.get(this.activeGroupId) : undefined;

		// per-turn 配置保留兼容名称，但可见边界按“连续工具阶段”处理：
		// 没有正文时，后续 assistant message 仍属于当前 Tools 账；
		// 正文出现后，下一次新的工具调用才开启下一本账。
		if (calls.length === 0) {
			if (hasNarration && activeGroup?.hasSeenToolBatch) {
				this.toolPhaseBoundaryPending = true;
			}
			return;
		}
		const assistantId = aggregateAssistantTurnId(message);
		const isSameAssistantMessage = assistantId !== undefined && assistantId === this.lastAssistantToolMessageId;
		const firstNewCall = calls.find((call) => !this.membersById.has(call.id));
		if (!firstNewCall || isSameAssistantMessage) return;
		const startsAfterBoundary =
			this.toolPhaseBoundaryPending || (hasNarration && activeGroup?.hasSeenToolBatch === true);
		if (startsAfterBoundary) {
			this.startUserGroup(`tool-phase:${firstNewCall.id}`, messageTimestampMs(message), {
				interruptUnsettled: false,
				continueToolPhase: true,
			});
			this.toolPhaseBoundaryPending = false;
		}
		// startUserGroup resets phase state. Record the source message only after a
		// possible phase switch so later streaming updates join this same ledger.
		this.lastAssistantToolMessageId = assistantId;
	}
...
export function padAggregateBlock(lines: readonly string[]): string[] {
	// Leave one separator after the low-priority ledger so the next assistant narration
	// starts as a distinct paragraph without restoring the old double-gap layout.
...
export function renderAggregateActivity(
	...
	const chromeColor = hasFailure ? "error" : view.hasRunning ? "warning" : "muted";
	const totals = theme.fg(
		"muted",
		` (${view.callCount} call${view.callCount === 1 ? "" : "s"} · ${view.agentTurnCount} turn${view.agentTurnCount === 1 ? "" : "s"})`,
	);
	const title = hasFailure || view.hasRunning
		? theme.fg("toolTitle", theme.bold?.("Tools") ?? "Tools")
		: theme.fg("muted", "Tools");
	let header = `${theme.fg(chromeColor, marker)} ${title}${totals}`;
	if (!view.settled && typeof view.startedAtMs === "number") {
		const elapsedMs = Math.max(0, Date.now() - view.startedAtMs);
		if (Math.round(elapsedMs / 1000) >= 1) {
			// 开放账本的 ticking elapsed 单独用 accent，避免和 muted 的 call/turn 统计糊在一起。
			header += theme.fg("muted", " · ");
			header += theme.fg("accent", formatAggregateDuration(elapsedMs));
		}
	}
	if (hasFailure) header += theme.fg("error", ` · ${view.failedCount} failed`);
	for (const summary of view.toolSummaries) {
		header += theme.fg("muted", " · ");
		header += theme.fg(chromeColor, `${summary.toolName} ×${summary.count}`);
	}
...
export function patchAggregateToolExecutions(projection: AggregateProjection): void {
	const claimedHost = claimHostProjection(undefined, projection);
	...
	if (existing) {
		if (prototype.render === existing.patchedRender || existing.projection !== undefined) {
			// The shared prototype survives runtime rebinding; move its fallback to
			// the newly claimed UI host while call-id lookup still serves children.
			if (claimedHost) existing.projection = projection;
			return;
		}
...
export function registerAggregateProjectionEvents(
	pi: ExtensionAPI,
	projection: AggregateProjection,
	options: { doneSettleDelayMs?: number; disposableScope?: DisposableScope } = {},
): void {
	...
	let openElapsedTimer: ReturnType<typeof setInterval> | undefined;
	const clearOpenElapsedTimer = () => {
		if (openElapsedTimer !== undefined) clearInterval(openElapsedTimer);
		openElapsedTimer = undefined;
	};
	const ensureOpenElapsedTimer = () => {
		// 开放账本的 elapsed 属于 display-intent：只有当前存在可渲染账本时才
		// 每秒失效重绘；普通用户组和 passthrough-only 组不会产生无意义的定时器。
		if (!projection.hasOpenLedger()) {
			clearOpenElapsedTimer();
			return;
		}
		if (openElapsedTimer !== undefined) return;
		openElapsedTimer = setInterval(() => {
			projection.invalidateOpenLedgers();
			if (!projection.hasOpenLedger()) {
				clearOpenElapsedTimer();
			}
		}, 1_000);
		openElapsedTimer.unref?.();
	};
	const rebuild = (ctx: SessionContextLike) => {
		clearSettleTimer();
		rebuildProjectionFromContext(projection, ctx);
		ensureOpenElapsedTimer();
	};
	...
	if (options.disposableScope) {
		options.disposableScope.registerCleanup(() => clearOpenElapsedTimer());
	}
