...
import { createDisposableScope, type DisposableScope } from "./disposable.js";
import {
...
import { BUILT_IN_TOOL_OVERRIDE_NAMES, usesAggregateLedger } from "./types.js";
...
const TOOL_DISPLAY_API_STACK_KEY = Symbol.for("pi-tool-display-intent.api-stack.v1");
const TOOL_DISPLAY_RUNTIME_OWNERS_KEY = Symbol.for("pi-tool-display-intent.runtime-owners.v1");
...
interface ToolDisplayApiRegistration {
  owner?: object;
  api: ToolDisplayApi;
}
...
type GlobalWithToolDisplayApi = typeof globalThis & {
  ...
  [TOOL_DISPLAY_API_STACK_KEY]?: ToolDisplayApiRegistration[];
  [TOOL_DISPLAY_RUNTIME_OWNERS_KEY]?: WeakSet<object>;
...
interface ToolDecorationFrame {
  owner: object;
  previous: ToolPropertyDescriptorSnapshot;
  applied: ToolPropertyDescriptorSnapshot;
}

const toolDecorationFrames = new WeakMap<RuntimeToolDefinition, ToolDecorationFrame[]>();

function snapshotToolPropertyDescriptors(
  tool: RuntimeToolDefinition,
...
function applyToolPropertyDescriptors(
  tool: RuntimeToolDefinition,
  snapshot: ToolPropertyDescriptorSnapshot,
): void {
  for (const property of TOOL_DISPLAY_DECORATED_PROPERTIES) {
...
function recordToolDecoration(
  owner: object,
  tool: RuntimeToolDefinition,
  previous: ToolPropertyDescriptorSnapshot,
  decoratedTools: Set<RuntimeToolDefinition>,
): void {
  const frames = toolDecorationFrames.get(tool) ?? [];
  frames.push({ owner, previous, applied: snapshotToolPropertyDescriptors(tool) });
  toolDecorationFrames.set(tool, frames);
  decoratedTools.add(tool);
}


function restoreOwnedToolDecorations(
  owner: object,
  decoratedTools: Set<RuntimeToolDefinition>,
): void {
  for (const tool of decoratedTools) {
    const frames = toolDecorationFrames.get(tool);
    const frameIndex = frames?.findIndex((frame) => frame.owner === owner) ?? -1;
    if (!frames || frameIndex < 0) continue;

    const [frame] = frames.splice(frameIndex, 1);
    const nextFrame = frames[frameIndex];
    if (nextFrame) nextFrame.previous = frame.previous;

    if (frameIndex === frames.length) {
      const survivingTop = frames.at(-1);
      applyToolPropertyDescriptors(tool, survivingTop?.applied ?? frame.previous);
    }

    if (frames.length === 0) toolDecorationFrames.delete(tool);
  }
...
function applyToolDisplayDecorationInPlace(
  tool: RuntimeToolDefinition,
  api: ToolDisplayApi,
  adapter: ToolDisplayAdapter | undefined,
  owner: object,
  decoratedTools: Set<RuntimeToolDefinition>,
  finalizeDecoration?: () => void,
): boolean {
  const previous = snapshotToolPropertyDescriptors(tool);
  try {
    Object.assign(tool, api.decorateTool(tool, adapter));
    finalizeDecoration?.();
    recordToolDecoration(owner, tool, previous, decoratedTools);
    return true;
  } catch (error) {
    applyToolPropertyDescriptors(tool, previous);
    logToolDisplayDebug("Tool display decoration failed.", error);
    return false;
  }
}

function drainPendingToolDisplayDecorations(
  api: ToolDisplayApi,
  owner: object,
  decoratedTools: Set<RuntimeToolDefinition>,
): void {
...
function publishToolDisplayApi(owner: object, api: ToolDisplayApi): void {
  const globalWithApi = globalThis as GlobalWithToolDisplayApi;
  let registrations = globalWithApi[TOOL_DISPLAY_API_STACK_KEY];
  if (!registrations) {
    registrations = [];
    const existingApi = globalWithApi[TOOL_DISPLAY_API_KEY];
    if (existingApi) registrations.push({ api: existingApi });
    globalWithApi[TOOL_DISPLAY_API_STACK_KEY] = registrations;
  }

  const previousIndex = registrations.findIndex((entry) => entry.owner === owner);
  if (previousIndex >= 0) registrations.splice(previousIndex, 1);
  registrations.push({ owner, api });

  const owners = globalWithApi[TOOL_DISPLAY_RUNTIME_OWNERS_KEY] ?? new WeakSet<object>();
  owners.add(owner);
  globalWithApi[TOOL_DISPLAY_RUNTIME_OWNERS_KEY] = owners;
  globalWithApi[TOOL_DISPLAY_API_KEY] = api;
}

function unpublishToolDisplayApi(owner: object, api: ToolDisplayApi): void {
  const globalWithApi = globalThis as GlobalWithToolDisplayApi;
  const registrations = globalWithApi[TOOL_DISPLAY_API_STACK_KEY] ?? [];
  const index = registrations.findIndex((entry) => entry.owner === owner && entry.api === api);
  if (index >= 0) registrations.splice(index, 1);

  if (!registrations.some((entry) => entry.owner === owner)) {
    globalWithApi[TOOL_DISPLAY_RUNTIME_OWNERS_KEY]?.delete(owner);
  }
  if (globalWithApi[TOOL_DISPLAY_API_KEY] === api) {
    const fallback = registrations.at(-1)?.api;
    if (fallback) globalWithApi[TOOL_DISPLAY_API_KEY] = fallback;
    else delete globalWithApi[TOOL_DISPLAY_API_KEY];
  }

  if (registrations.length === 0) {
    delete globalWithApi[TOOL_DISPLAY_API_STACK_KEY];
    delete globalWithApi[TOOL_DISPLAY_RUNTIME_OWNERS_KEY];
  } else if (!registrations.some((entry) => entry.owner)) {
    delete globalWithApi[TOOL_DISPLAY_RUNTIME_OWNERS_KEY];
  }
}
...
function installToolDisplayApi(
  ...
  const api: ToolDisplayApi = {
    version: 1,
    decorateTool<T extends RuntimeToolDefinition>(tool: T, adapter?: ToolDisplayAdapter): T {
      if (usesAggregateLedger(getConfig().toolCallLayout)) return tool;
  ...
  const globalWithApi = globalThis as GlobalWithToolDisplayApi;
  publishToolDisplayApi(owner, api);
  if (usesAggregateLedger(getConfig().toolCallLayout)) {
    ...
    drainPendingToolDisplayDecorations(api, owner, decoratedTools);
  }
  return api;
}
...
export function registerToolDisplayOverrides(
  pi: ExtensionAPI,
  getConfig: ConfigGetter,
  disposableScope: DisposableScope = createDisposableScope(),
): void {
  clearBuiltInToolCache();
  const decoratedTools = new Set<RuntimeToolDefinition>();
  const unregisterLiveToolCalls = registerLiveToolCallEvents(pi);
  disposableScope.registerCleanup(unregisterLiveToolCalls);
  const toolDisplayApi = installToolDisplayApi(
    pi,
    getConfig,
    decoratedTools,
  );
  disposableScope.registerCleanup(() => {
    restoreOwnedToolDecorations(pi, decoratedTools);
    unpublishToolDisplayApi(pi, toolDisplayApi);
  });
  ...
  const layout = getConfig().toolCallLayout;
  const aggregateProjection = usesAggregateLedger(layout)
    ? new AggregateProjection(
        ...
        layout === "per-turn" ? "tool-phase" : "user",
      )
    : undefined;
  ...
  disposableScope.registerCleanup(() => wrappedCustomToolNames.clear());
  ...
  disposableScope.registerCleanup(() => wrappedMcpToolNames.clear());
  ...
  if (aggregateProjection) {
    registerAggregateProjectionEvents(pi, aggregateProjection, { disposableScope });
  }
}
