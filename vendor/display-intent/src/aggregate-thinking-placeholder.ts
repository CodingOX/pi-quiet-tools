...
interface AggregateThinkingOwner {
	isAggregateEnabled: () => boolean;
	hasUi?: boolean;
}

const thinkingOwnersByApi = new WeakMap<ExtensionAPI, AggregateThinkingOwner>();
const thinkingOwners = new Set<AggregateThinkingOwner>();
let activeThinkingOwner: AggregateThinkingOwner | undefined;
...
function activateThinkingOwner(owner: AggregateThinkingOwner): void {
	activeThinkingOwner = owner;
	patchAggregateThinkingPlaceholders(owner.isAggregateEnabled);
}

function relinquishThinkingOwner(owner: AggregateThinkingOwner): void {
	if (activeThinkingOwner !== owner) return;
	activeThinkingOwner = undefined;
	const fallback = [...thinkingOwners].reverse().find((candidate) => candidate.hasUi === true);
	if (fallback) {
		activateThinkingOwner(fallback);
	} else if (thinkingOwners.size === 0) {
		restoreAggregateThinkingPlaceholders();
	} else {
		patchAggregateThinkingPlaceholders(() => false);
	}
}
...
export function registerAggregateThinkingPlaceholderSuppression(
	...
	let owner = thinkingOwnersByApi.get(pi);
	if (owner) {
		owner.isAggregateEnabled = isAggregateEnabled;
	} else {
		owner = { isAggregateEnabled };
		thinkingOwnersByApi.set(pi, owner);
		thinkingOwners.add(owner);
	}
	if (!activeThinkingOwner || activeThinkingOwner === owner) activateThinkingOwner(owner);

	onSessionShutdown(pi, () => {
		if (thinkingOwnersByApi.get(pi) !== owner) return;
		thinkingOwnersByApi.delete(pi);
		thinkingOwners.delete(owner);
		relinquishThinkingOwner(owner);
		if (thinkingOwners.size === 0) restoreAggregateThinkingPlaceholders();
	});
	const refreshOwner = (ctx?: { hasUI?: boolean }): void => {
		if (ctx?.hasUI !== undefined) owner.hasUi = ctx.hasUI !== false;
		if (owner.hasUi !== false) activateThinkingOwner(owner);
		else relinquishThinkingOwner(owner);
	};
	pi.on("session_start", async (_event, ctx) => refreshOwner(ctx));
	pi.on("before_agent_start", async (_event, ctx) => refreshOwner(ctx));
}
