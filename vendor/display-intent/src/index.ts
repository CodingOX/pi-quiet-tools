...
import { createDisposableScope } from "./disposable.js";
...
export default function toolDisplayExtension(pi: ExtensionAPI): void {
  ...
  const disposableScope = createDisposableScope();
  ...
  registerToolDisplayOverrides(pi, getEffectiveConfig, disposableScope);
  registerAggregateThinkingPlaceholderSuppression(
    pi,
    () => usesAggregateLedger(initial.config.toolCallLayout),
  );
