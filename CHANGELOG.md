...
---
## 2026-09-19 · 看门狗：提示条改走 dim 状态行，nudge 文案加一分专注自查

第 80 次 bash 的 nudge 指令连调 80 次仍没出结果，往往不是任务真的更大，而是模型**在跑偏时
不自觉**。因此指令的汇报项从两项扩到三项，中间插一条自我确认：

```text
1) 正在做什么；
2) 是否仍专注于目标、有没有过度发散；   ← 新增
3) 还差哪些步骤就能收口。              ← 原「还差哪一步」
```

**「哪一步」→「哪些步骤」**：原文把收口路径隐含限定为单步，模型可能据此认为「说不出一条
就还没到收口点」，于是继续堆调用。改成分量词后，多步骤的收尾清单也能如实报出。

### Changed

- **nudge 指令加「专注自查」并放宽收口口径**（`packages/watchdog/src/host-watchdog.ts`）。
  阈值（80 / 10）、状态机、开口即重置的语义一律未动 —— 这次只动文案。
  断言同步（`host-watchdog.test.ts`）：锁住 `/发散/` 与 `/还差哪些步骤/`。
- **人类可见的提示条不再用 warning severity**（`packages/watchdog/src/host-watchdog.ts` 的
  `announce()`）。原先传 `"warning"`，被 Pi 的 `interactive-mode` 渲染成满亮黄
  （dark 主题 `#ffff00`）+ 硬编码的 `"Warning: "` 前缀，比正文还扎眼。改传 `"info"` 走
  `showStatus`，即 dim 灰、无前缀。
- **nudge 提示文案从「请尽快收口」改成陈述句**「bash 过多，已请模型汇报进度」。
  旧句和第一档指令打架：指令明说汇报后可以继续，提示条却在催收尾。

> 提示条（`NUDGE_NOTIFY` / `HARD_STOP_NOTIFY`）与注入模型的指令（`NUDGE_INSTRUCTION` /
> `HARD_STOP_INSTRUCTION`）是两条独立通道：前者走 `ctx.ui.notify` 只给人看，不进上下文；
> 后者走 `context` 事件注入。改一条不必动另一条。

断言同步：新增一条锁 severity 的用例，nudge 必须 `type === "info"`；`hostCtx` 的
notify 签名从 `(message) => void` 放宽为带 type，以便记录 severity。
## 2026-09-18 · 账本表头下的凭空空行

折叠账本里 `Tools (…)` 表头与首个工具行之间总是空着一整行，看起来像「间距过宽」。
它不是设计间隔，而是一行**内容为空的旁白钉残留**：display-intent 的折叠旁白钉走 Markdown
渲染，而 Markdown **把每一行补齐到整行宽度**，于是旁白的段落分隔行到达胶水层时是
`    ` + 一整行空格 —— `visibleWidth` 不为 0，上游 `trimRenderedEdges` 剪不掉；
`isCollapsedNarrationContinuation` 又要求 `trim()` 非空，于是该行被当成「不是旁白行」
**保留**，并中断了续行跳过链。
...
| 宽度 | 首段 | 泄漏内容 |
| :--- | :--- | :--- |
| 窄（≈60 列） | 折成两行 | 只剩那行填充空白 → 表头与首行之间的宽间距 |
| 宽（≥100 列） | 单行 | 填充空白 **+** 旁白的下一段正文（如 `我的理解`）一起漏进账本 |

### Fixed

- **填充空行也算旁白续行**（`src/aggregate-omit-ledger-narration.ts`）。
  判定从「`trim()` 非空」放宽为「整行空白即续行」，于是整条折叠钉（含折行、段落分隔、
  后续段落）被完整剔除，表头下一行直接是首个工具行。
  账本自带的首尾间隔（`padAggregateBlock`）与**不跟在旁白钉后面**的缩进空行原样保留，
  后者由新增的反向测试锁住。

定位证据：截图像素量测（表头→首行 88px vs 行距 43px，中间那一行只有 2 个杂点像素）、
sage-flow 会话（`11 calls · 6 turns · bash ×6 · read ×5`）逐行复算，以及 width 58/62/66/120
四个宽度下的前后对照。
