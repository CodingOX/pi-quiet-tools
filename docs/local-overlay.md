...
**Ledger left-edge indent** — `aggregate-ledger-indent.ts`

Pi renders assistant prose through `AssistantMessageComponent`, whose left edge is pushed out by `outputPad` (default 1). display-intent paints the `Tools` header at column 0, so the header sat one column *left* of the prose it belongs to. Glue shifts ledger rows right by one so the header lines up with the prose, keeping the block's internal hierarchy (header 0 / stats 2 / rows 2) intact — a pure translation. Rows are classified one at a time rather than block-shifted, because the expanded (`Ctrl+O`) path renders ledger rows and recovered prose in the same block; the one exception is an expanded steer, whose rail block has no marker on its continuation lines and is therefore shifted whole. The shift is idempotent (only base-indent rows move) since prototype wrappers survive `/reload`. Three hosts: `aggregate-silent-tools.ts` (tool-execution prototype), `aggregate-keep-narration.ts` (assistant-message prototype, expanded header), and `aggregate-steer-indent.ts` (user-message prototype, expanded steer — display-intent renders that block on `UserMessageComponent`, which the other two never see).
