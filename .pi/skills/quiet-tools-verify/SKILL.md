...
- host nudge → 终端出现一行**dim 灰**中文提示（`看门狗：bash 过多，已请模型汇报进度`），**没有** `Warning: ` 前缀、**不是**满亮黄；它是 `notify(..., "info")` 走 `showStatus`，与注入模型的指令是两条独立通道；
- host 写**可见中文正文**（不是 thinking）后，bash 计数与宽限**应当重置**；
- host 继续沉默调 bash → 宽限 10 回合用尽后后续工具被拦，并被要求用中文说明「当前做到哪 / 建议下一步」；
- 子会话（`hasUI !== true`）**有自己的 80+10 账本**，不和 host 混算；子会话可见正文**不重置**；硬停文案是给父代理的 `INCOMPLETE` 未完成交接，不是给用户的播报；子会话不会自己 resume；
- 子会话 nudge **不会** `ctx.ui.notify`；
- 正在跑的命令**不会**被中止；
- Ask / 子代理干等**不会**绊 host 账本。
